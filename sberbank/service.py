...
from order.models import Order, Status as order_status
...
class BankService(object):
    ...
    def pay(self, amount, order_id, preauth=False, **kwargs):
        ...
        payment = Payment(amount=amount, client_id=client_id, method=Method.WEB, order_id=order_id, details={
            'username': self.merchant.get("username"),
            'currency': currency,
    ...
    def execute_request(self, data, method, payment=None):
        ...
        try:
            print('{}/{}.do'.format(self.__default_gateway_address, method), data, headers)
